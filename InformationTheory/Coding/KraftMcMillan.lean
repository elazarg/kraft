...
lemma sum_pow_len_filter_le_one_of_card_le [Fintype α] [Nonempty α]
    {T : Finset (List α)} {s : ℕ}
...
private lemma kraft_sum_pow_eq_sum_concatFnS {S : Finset (List α)} {D : ℕ} {r : ℕ} :
  ...
  have h_term (w : Fin r → S) :
    ...
    simpa [concatFnS.length w] using (hprod w)
  calc
...
lemma kraft_mcmillan_inequality_aux {S : Finset (List α)} [Fintype α] [Nonempty α]
  ...
  refine le_trans kraft_sum_pow_eq_sum_concatFnS.le
    <| h_injective.trans
...
public theorem kraft_mcmillan_inequality {S : Finset (List α)} [Fintype α] [Nonempty α]
  ...
  have hr_exists : Filter.Tendsto (fun r : ℕ =>
    ...
    have h0 :
      ...
      exact tendsto_mul_const_nat_div_pow_atTop_nhds_0 (by simpa [K] using h_kraft)
