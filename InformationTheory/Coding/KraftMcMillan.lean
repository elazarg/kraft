...
private lemma kraft_mcmillan_inequality_aux_of_concatInj
      ...
      · intro a _
        ...
        · -- r ≤ (concatFn a).length
          ...
          have hsum : (∑ _ : Fin r, 1) ≤ ∑ i, ((a i).val).length := by
            ...
            have hi_pos : 1 ≤ ((a i).1).length := one_le_length_of_mem hε _ (a i).prop
...
public theorem kraft_mcmillan_inequality_of_concatInj
  ...
  have h_kraft := kraft_mcmillan_inequality_aux_of_concatInj (α := α) (S := S) hε hinj
  -- then literally reuse your existing `contrapose!` + `Tendsto` proof
  contrapose! h_kraft
