...
private lemma prefixFree_of_div_separated {k : ℕ} {l : Fin k → ℕ} {A : ℕ → ℕ}
    ...
    PrefixFree (Set.range (fun i : Fin k => kraftCodeword hD (A i.val) (l i))) := by
  intro a ⟨i, qi⟩ b ⟨j, qj⟩ hpre
  by_cases hij : (i = j) <;> subst a b
  · subst i
    rfl
  · obtain ⟨hle, hdiv⟩ := (kraftCodeword_prefix_iff_div hD (h_bound i) (h_bound j)).mp hpre
...
theorem exists_code_of_strict_prefix_sum
  ...
  let w(n: ℕ) : List (Fin D) := kraftCodeword hD (A n) (l n)

  have hw_len : ∀ n, (w n).length = l n := by
    simp [w]

  refine ⟨w, ?_, ?_, (by simp [w, hw_len])⟩

  · -- Injectivity
    intro n m hnm
    have hlen : l n = l m := by
      -- lengths of equal lists are equal
      simpa [hw_len n, hw_len m] using congrArg List.length hnm

    have : A n = A m := by
      -- rewrite the second bound using hlen
      refine kraftCodeword_inj hD (hA_bound n) ?_ ?_
      · simpa [hlen] using hA_bound m
      · simpa [w, hlen] using hnm
  ...
  · -- Prefix-freeness
    ...
    · -- turn prefix into the division characterization
      have hdiv :
        ...
        (kraftCodeword_prefix_iff_div hD (hA_bound n) (hA_bound m)).1 hpre
...
theorem exists_code_fin
  ...
  -- 4. Define code
  let w(i : Fin k) : List (Fin D) := kraftCodeword hD (A i.val) (l i)
  ...
  · intro i j hij
    ...
    have hA_eq : A i.val = A j.val := kraftCodeword_inj hD (hA_bound i)
      (by simpa [hlen] using hA_bound j)
      (by rw [<-hlen] at hij; exact hij)
    exact hA_strict.injective hA_eq

  -- Prefix-freeness
  · suffices PrefixFree (Set.range (fun i : Fin k => kraftCodeword hD (A i.val) (l i))) by
      simpa [w] using this
