...
\begin{itemize}[leftmargin=1.5em,itemsep=1pt]
  ...
  \item \textbf{Source coding lower bound.} For any injective uniquely
    decodable code $w$ and nonnegative distribution $p$,
    \[
...
convention into \texttt{Real.negMulLog}. The formalization connects the
finite sum to \textsf{Mathlib}'s measure-theoretic \texttt{klDiv} through
\texttt{pmfMeasure} in a separate divergence module, while the source-coding
theorem itself uses the elementary, zero-mass-tolerant
\texttt{klFin\_nonneg}. The normalized Kraft
weights satisfy $q_i>0$, so absolute continuity is automatic even when
$p_i=0$. The public theorem therefore assumes only $p_i\geq 0$; its
weighted logarithm calculation treats the $p_i=0$ case separately.
