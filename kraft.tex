...
Therefore
\[
 \sum_{i=1}^{i_0} 2^{\ell'_{i_0} - \ell'_i} = 2^{\ell'_{i_0}},
\]
and so
\[
...
\]
Therefore \Cref{lem:aux} implies that there exists a set $S \subseteq I$ such that
\[
...
Therefore we can apply the induction hypothesis twice, on $\ell'|_S$ and on $\ell'|_{S^c}$, to obtain injective maps $w_0\colon S \to \{0,1\}^*$ and $w_1\colon S^c \to \{0,1\}^*$ such that $w_0(S)$ and $w_1(S^c)$ are prefix-free; $|w_0(i)| = \ell'(i)$ for all $i \in S$; and $|w_1(i)| = \ell'(i)$ for all $i \in S^c$.
...
A finite set $S$ of words is \emph{uniquely decodable} if every $x \in \{0,1\}^*$ can be written in at most one way as $x = w_1 \dots w_r$ (for any $r$), where $w_1,\dots,w_r \in S$.
\end{definition}
...
\[
 ...
 \sum_{s=r}^{r\ell} 2^s 2^{-s} = r\ell - r + 1 \leq r\ell.
\]
...
and so
\[
 \log_2 C \leq \frac{\log_2 r + \log_2 \ell}{r}.
\]

Let $r = 2^{2t}$. Then
\[
 \log_2 r = 2t < 2 \cdot 2^t = 2^{t+1}.
\]
...
Prefix-free codes are uniquely decodable (the converse doesn't hold: for example, $\{10,100\}$ is uniquely decodable since it is suffix-free, but is not prefix-free since $10$ is a prefix of $100$).
...
Suppose without loss of generality that $|w_1| \leq |z_1|$. We claim that $w_1$ is a prefix of $z_1$. Indeed, $z_1 = w_1 q$, where $q = x_{|w_1|+1} \dots x_{|z_1|}$. Since $S$ is prefix-free, $w_1 = z_1$. This implies that $w_2 \dots w_r = z_2 \dots z_s$. Denote the common value by $y$.
